...
if __name__ == "__main__":
    num_workers = 4  # Number of worker processes,  TODO: make config for this
    input_queue = mp.Queue()  # Create a queue to store tasks

    # Create multiple pipes for communication with the worker processes
    output_pipes = [mp.Pipe() for _ in range(num_workers)]

    workers = []  # List to store worker processes

    # Start the worker processes
    for i in range(num_workers):
        # Create a worker process, passing the input queue and output pipe as arguments
        worker_process = mp.Process(
            target=worker, args=(input_queue, output_pipes[i][1])
        )
        worker_process.start()  # Start the worker process
        workers.append(worker_process)  # Add the worker process to the list

    tasks = []  # List of tasks to be processed

    # Distribute tasks to the workers
    for task in tasks:
        input_queue.put(task)  # Put each task into the input queue
    ...
    for i in range(num_workers):
        result = output_pipes[i][0].recv()  # Receive the result from the output pipe
    ...
    for result in results:
        process_result(result)  # Call the process_result function to handle each result
